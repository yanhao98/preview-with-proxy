...
const form = await (async () => {
  ...
  const previewPort = env.VITE_PREVIEW_PORT || "3000";
  const buildCommand = `npm run ${env.VITE_PREVIEW_BUILD_SCRIPT || "build"}`;
  ...
  const result = await Enquirer.prompt({
    ...
    choices: [
      { message: "预览端口", name: "previewPort", initial: previewPort }, //
      { message: "构建命令", name: "buildCommand", initial: buildCommand }, //
      {
        message: "预览代理",
...
async function runBuild() {
  ...
  buildPromise = execaCommand(form.buildCommand, {
    stdio: "pipe",
  });
  await oraPromise(buildPromise, {
    text: `正在执行构建命令: ${chalk.cyan(form.buildCommand)}`,
